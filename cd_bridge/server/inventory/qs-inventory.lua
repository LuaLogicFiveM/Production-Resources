...
function AddQualityToItem(source, item_name, hp, slot)
    local inventory = exports['qs-inventory']:GetInventory(source)
    if slot ~= nil then
        inventory[slot].info.durability = hp
    end
    for _, item in pairs(inventory) do
        if item.name == item_name and item.durability == nil then
            item.info.durability = hp
            return
        end
    end
    return nil
...
function GetItemQuality(source, item_name, slot)
    local inventory = exports['qs-inventory']:GetInventory(source)
    if slot ~= nil then
        return inventory[slot].info.durability
    end
    for _, items in pairs(inventory) do
        if item.name == item_name then
            return item.info.durability
        end
    end
    return nil
end
...
function SetItemQuality(source, item_name, hp, slot)
    ...
    for _, item in pairs(PlayerItems) do
        if item.name == item_name and item.info.durability ~= nil and item.info.durability > 0 then
            item.info.durability = hp
            return
        end
    end
