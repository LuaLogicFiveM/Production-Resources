...
function Locale(locale_key, ...)
    ...
    local message = (LocalesTable[lang] and LocalesTable[lang][locale_key]) or (LocalesTable['EN'] and LocalesTable['EN'][locale_key])
    ...
    local ok, formatted = pcall(string.format, message, table.unpack(args))
    if ok then
        return formatted
    end

    ERROR('0082',('String format failed for locale key: %s | Args: %s\n'):format(locale_key, json.encode(args)))
