...
if (IsDuplicityVersion()) {
    ...
    global.CreateCamera = (...args) => {
        const camera = _CreateCamera(...args);
        SetState(`CustomCam:${camera}`, true);
        SetState(`CamChange`, GetGameTimer());
        return camera;
    };

    global.CreateCamWithParams = (...args) => {
        const camera = _CreateCamWithParams(...args);
        SetState(`CustomCam:${camera}`, true);
        SetState(`CamChange`, GetGameTimer());
        return camera;
    };
    ...
    global.CreateCameraWithParams = (...args) => {
        const camera = _CreateCameraWithParams(...args);
        SetState(`CustomCam:${camera}`, true);
        SetState(`CamChange`, GetGameTimer());
        return camera;
    };
