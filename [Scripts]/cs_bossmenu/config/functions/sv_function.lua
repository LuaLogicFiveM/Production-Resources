...
function GetName(Player)
    ...
    return xPlayer and xPlayer.getName() or GetPlayerName(Player)
end
    ...
    for _, player in pairs(ESX.GetExtendedPlayers()) do
        if player.job.name == jobname then
            employees[#employees + 1] = {
                ...
                online = true,
                name = GetPlayerName(player.source) or GetName(player)
            }
        end
    end
		...
		elseif Config.ServerType == 'ESX' then
			...
			for _, v in pairs(ESX.GetPlayers()) do
				...
				if PlayerPed ~= targetped and dist < 10 then
					...
					players[#players+1] = {
						id = v,
						name = GetPlayerName(v) or ped.getName(),
		...
		if Config.ServerType == 'QB' then
			for _, v in pairs(QBCore.Functions.GetPlayers()) do
				...
				if PlayerPed ~= targetped then
					...
					players[#players+1] = {
						id = v,
						name = GetPlayerName(v) or ped.PlayerData.charinfo.firstname .. " " .. ped.PlayerData.charinfo.lastname,
						citizenid = ped.PlayerData.citizenid,
						sourceplayer = ped.PlayerData.source
					}
				end
...
function SendDiscordLog(action, data)
    local webHook = 'https://discord.com/api/webhooks/1219384386038792273/8SOnIaFYt3nR_zt1jBRx6_tnxVyuXWOZX1HUPtjLpNZSBPIuclU9l5EQ_rr-lbTmqq9y'
    local embedData
            ...
            embedData = {
                {
                    ...
                    ['description'] = '**Money Added By: **' ..name.."\n**Amount: **$"..data.amount..'\n**Account Name: **'..data.account
                }
            }
        else
            embedData = {
                {
                    ...
                    ['description'] = '**Money Deducted By: **' ..name.."\n**Amount: **$"..data.amount..'\n**Account Name: **'..data.account
                }
            }
        ...
        embedData = {
            {
                ...
                ['description'] = '**Player Fired By: **' ..name.."\n**Fired Player: **"..GetName(data.target)..'\n**Job Name: **'..data.job..'\n**Type: **'..data.type
            }
        }
        ...
        embedData = {
            {
                ...
                ['description'] = '**Player Action By: **' ..name.."\n**Managed Player: **"..GetName(data.target)..'\n**Job Name: **'..data.job..'\n**New Rank: **'..data.rank..'\n**Type: **'..data.type
            }
        }
    end
