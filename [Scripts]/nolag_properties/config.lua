Config = {
    RealEstateJobs = {
        ...
        ["realestate"] = {
            ["creation"] = 1,                -- Can create new properties
            ["allproperties"] = 1,           -- Can view all properties in admin panel
            ["deleteproperty"] = 1,          -- Can delete properties
            ["deletebuilding"] = 2,          -- Can delete buildings
            ["forceopeninventory"] = 2,      -- Can force open property inventories
    ...
    CreatePropertyCommand = "propertycreate",   -- The command to create a property
    ...
    --[[
        ...
        * fivem-appearance
        * rcore_clothing
        * rcore_clothes (The old version of rcore_clothing)
        * vms_clothestore
    ...
    FurnitureModelTimeout = 10000,           -- The time to wait for furniture models to load with UI indicator (default: 10 seconds, min: 5000, max: 30000)
    -- When placing furniture from the menu, a loading indicator will show while the model loads
    -- If the model fails to load within this time, the placement will be cancelled
    ...
    PointSelectionMethods = {
        enterPoint = "ped",         -- Enter point selection method
        exitPoint = "ped",          -- Exit point selection method (spawn coords)
        mloInteractPoint = "ped",   -- MLO interaction point selection method
        interactablePoints = "ped", -- General interactable points selection method
    },

    --[[ Properties ]]
    --[[ Lockpick Configuration ]]
    Lockpick = {
        Enable = true,                 -- Enable lockpicking by other players (default: false)
        RequireOwnerOnline = true,    -- Require the property owner to be online to lockpick the property (default: false)
        AllowCompanyProperties = false, -- Allow lockpicking of company/society owned properties (default: true)

        Item = {
            Require = true,         -- Require a lockpick item to lockpick (default: false)
            Name = 'lockpick_house',       -- The name of the lockpick item
            Amount = 1,              -- The amount of lockpick items required
            RemoveOnFail = true,     -- Remove the lockpick item on failed attempt (default: true)
            RemoveOnSuccess = false, -- Remove the lockpick item on successful attempt (default: false)
        },

        --- Lockpick minigame function
    ...
    StarterApartment = {
        ...
        DisableFurniture = true,    -- Disable furniture functionality for starter apartments
        ...
        RentedInstead = false,       -- If true, the apartment will be rented instead of owned
        InitialRentDays = 7,        -- How many days the starter apartment should be rented for free
        FutureRentPrice = 10000,      -- How much it will cost to renew the rent after the free period
        Interior = {
            type = "shell",         -- The type of the interior, "shell" or "ipl"
            name = "Container",     -- The name of the shell or ipl
        },
        Inventory = {
            slots = 50,     -- The amount of slots the inventory has
            weight = 50000, -- The weight of the inventory
        },
        InteractablePoints = {
    ...
    EnableMap = true, -- Enable the leaflet map displayed in the the creation menu
    ...
    DisableSocietyBuying = true,  -- Disable buying properties as a society
    ...
    EnableForceSale = false,           -- Enable force saling of properties
    RentPercentage = 100,              -- How much money will the owner get from the rent (default: 90 %)
    MaxRentDays = 30,                 -- Maximum number of days a property can be rented for (default: 10)
    ...
    EnableSellProperty = false,

    -- [[ Ipls ]]
    IplPrice = { 1000000, 10000000 }, -- The price of the ipl, first number is the minimum, second is the maximum.
    ...
    IplsUseRoutingBuckets = true, -- Use routing buckets instead of concealing for ipls (concealing allows to see the outside world when in ipl with windows but could be problematic for some people)


    --[[ Shells ]]
    ShellZLevel = 2000.0,        -- The z level of the shell spawn (default: 2000)
    ShellPrice = { 100000, 10000000 }, -- The price of the shell, first number is the minimum, second is the maximum.
    --[[
    ...
    --[[ Mlos ]]
    MloPriceBasedOnArea = false,    -- Should the price be based on the area of the mlo?
    MloInventoryData = {
        Slots = { 50, 100 },        -- The amount of slots the inventory has, first number is the minimum, second is the maximum
        Weight = { 10000, 1000000 },   -- The weight of the inventory, first number is the minimum, second is the maximum
    },
    PricePerSquare = { 1, 200 },   -- The price per square meter, first number is the minimum, second is the maximum
    MloPrice = { 10000000, 100000000 },     -- The price of the mlo, first number is the minimum, second is the maximum (if MloPriceBasedOnArea is false)
    ...
    RestrictFreecamToZone = false, -- Restrict freecam movement to stay within property/building zone boundaries (prevents abuse)
