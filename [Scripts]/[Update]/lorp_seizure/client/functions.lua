...
lib.onCache('vehicle', function(vehicleEntered, vehicleExited)
    local vehicle = vehicleEntered and vehicleEntered or vehicleExited
    local vehicleNetId = VehToNet(vehicle)

    if vehicle then
        TriggerServerEvent('dream_policeimpound:server:enteredVehicle', vehicleNetId)
    else
        TriggerServerEvent('dream_policeimpound:server:exitedVehicle', vehicleNetId)
    end
end)