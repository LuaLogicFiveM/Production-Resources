...
local function createOption(coords, option, id, width, showDot, alpha)
    ...
    utils.drawOption(coords, option.label, 'tw_plumber_txd', currentSelection == id and selected or unselected, id - 1, width, showDot, alpha)
end
...
local function CreateInteractions()
    ...
    for i = 1, nearbyAmount do
        ...
        if isPrimary and currentAlpha < 0 then
            ...
            DrawSprite('tw_plumber_txd', interact, 0, 0, 0.0185, 0.03333333333333333, 0, 255, 255, 255, alpha)
            ...
            DrawSprite('tw_plumber_txd', pin, 0, 0, 0.010, 0.025, 0, 255, 255, 255, isPrimary and currentAlpha or 255)
        end
...
local function isOnActiveJob()
    if (CoopDataClient and CoopDataClient.roomSetting) or (Config.InteractionHandler == "scriptbase" or Config.InteractionHandler == "drawtext") then
        return true
    else
        return false
    end
...
CreateThread(function ()
    RequestStreamedTextureDict('tw_plumber_txd')
    while not HasStreamedTextureDictLoaded('tw_plumber_txd') do Wait(100) end
