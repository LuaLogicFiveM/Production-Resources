...
-- Konfigürasyon
local Config = {
    SQLName = base.SQLName or 'tw_scrapyard',
    SQL = "oxmysql", -- oxmysql, ghmattimysql, mysql-async
    DefaultCharset = "utf8mb4",
    DefaultCollation = "utf8mb4_unicode_ci",
    LogPrefix = "[ TW SCRAPYARD V2 ]",
...
local TableSchemas = {
    -- Profil Verileri Tablosu
    {
        ...
        columns = {
            ...
            { name = "uisettings",   type = "longtext", nullable = true, default = "{\"soundEffect\":true,\"locale\":\"en\",\"uiPositions\":{\"teamList\":{\"left\":\"85.94vw\",\"top\":\"77.22vh\"},\"scoreList\":{\"left\":\"1.61vw\",\"top\":\"2.64vh\"},\"inviteSide\":{\"left\":\"73.07vw\",\"top\":\"2.85vh\"},\"notificationDiv\":{\"left\":\"81.54vw\",\"top\":\"40.48vh\"},\"keyInfoSide\":{\"top\":\"50%\",\"right\":\"2.0833vw\"},\"dumpsterInfo\":{\"top\":\"1.3542vw\",\"left\":\"50%\"}}}" },
            { name = "craftdata",    type = "longtext", nullable = true, default = "[]" },
        }
    },
}
    ...
    for _, column in ipairs(schema.columns) do
        ...
        if column.default ~= nil then
            if type(column.default) == "string" then
                -- CRITICAL: Escape % characters in default value to prevent gsub pattern errors
                local escapedDefault = column.default:gsub("%%", "%%%%")
                def = def:gsub("DEFAULT NULL", "DEFAULT '" .. escapedDefault .. "'")
            else
