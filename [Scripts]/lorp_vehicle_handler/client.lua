...
lib.callback.register('lorp_vehicle_handler:client:basicfix', function(fixtype)
    if not Handler then return end
    return Handler:basicfix(fixtype)
end)

lib.callback.register('lorp_vehicle_handler:client:basicwash', function()
    if not Handler then return end
    return Handler:basicwash()
end)

lib.callback.register('lorp_vehicle_handler:client:adminfix', function()
    if not Handler or not Handler:isActive() then return end
    return Handler:adminfix()
end)

lib.callback.register('lorp_vehicle_handler:client:adminwash', function()
    if not Handler or not Handler:isActive() then return end
    return Handler:adminwash()
end)

lib.callback.register('lorp_vehicle_handler:client:adminfuel', function(newlevel)
    if not Handler or not Handler:isActive() then return end
