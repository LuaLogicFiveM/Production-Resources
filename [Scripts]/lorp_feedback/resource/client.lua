...
local function feedbackInput()
    if not lib.callback.await('lorp_feedback:canSubmitForm') then 
    ...
    local feedback = lib.callback.await('lorp_feedback:submittedForm', false, input)
...
RegisterCommand(config.command, feedbackInput, false)
