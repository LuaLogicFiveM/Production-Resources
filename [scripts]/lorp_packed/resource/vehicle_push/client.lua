local config = lib.loadJson('resource.vehicle_push.shared')
local pressed = {
    e = false,
    shift = false
}
    ...
    if not pushingControl then -- initial loop for new owner
        ...
        CreateThread(function()
            ...
            if DoesEntityExist(vehicle) and NetworkGetEntityOwner(vehicle) ~= playerId and pushingControl then -- handle changing owner in the middle of pushing
                TriggerServerEvent('lorp_vehiclepush:server:push', {
...
local function pushVehicle()
    ...
    lib.playAnim(ped, 'missfinale_c2ig_11', 'pushcar_offcliff_m', 2.0, -8.0, -1, 35, 0, false, false, false)
...
lib.addKeybind({
    name = 'push_vehicle_e',
    description = 'Vehicle Push Combination',
    defaultKey = 'E',
    onPressed = function()
        pressed.e = true
