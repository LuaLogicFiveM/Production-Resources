...
window.addEventListener("message", function(event) {
	...
	if (item.showmenu) {
		...
		$("#nav-bar").empty();
		$("#nav-bar").append(`
			` + getPagination(config.pagination) + `
			<li onclick="closeUI()">
...
function openPage(pageN) {
	...
	if (pageN == 10) {
		$(".pages").css("display", "none");
		$(".market-page10").css("display", "block");
	}
...
function getPagination(pagination) {
	var html = "";
	// Adjust the loop to iterate over the full range of categories
	for (var i = 0; i <= Object.keys(pagination).length; i++) { // Starts from 1 (Lua index)
		if (pagination[i]?.name) {
			if (pagination[0] == undefined) {
				html += `
				<li class="sidebarClass sidebar-5" onclick="openPage(5)">
					<i class="fas ` + pagination[i].icon + `"></i>
					<span class="tooltip">` + pagination[i].name + `</span>
				</li>`;
			}else {
				html += `
				<li class="sidebarClass sidebar-` + i + `" onclick="openPage(` + i + `)"> <!-- Subtract 1 to match JS -->
					<i class="fas ` + pagination[i].icon + `"></i>
					<span class="tooltip">` + pagination[i].name + `</span>
				</li>`;
			}
...
function post(name, data) {
	$.post("https://ak47_leafnlatte/" + name, JSON.stringify(data), function(datab) {});
}
...
function renderCartItems() {
	...
	checkoutButton.addEventListener("click", () => {
		...
		if (cartItems.length > 0) {
			$.post("https://ak47_leafnlatte/buyItems", JSON.stringify({
				cart : cartItems,
    ...
    window.addEventListener('message', function(event) {
        ...
        async function playAudio(playdata){
            ...
            audioPlayer.on('end', function(){
                audioPlayer = null;
                $.post('http://ak47_leafnlatte/audioend', JSON.stringify({id:playId}));
            });
            ...
            while (audioPlayer != null) {
                $.post('http://ak47_leafnlatte/getvolume', JSON.stringify({
                    id: playId
                }), function(cb) {
                    if (audioPlayer != null) {
