...
function post(name, data) {
	$.post("https://ak47_khusbites/" + name, JSON.stringify(data), function(datab) {});
}
...
function renderCartItems() {
	...
	checkoutButton.addEventListener("click", () => {
		...
		if (cartItems.length > 0) {
			$.post("https://ak47_khusbites/buyItems", JSON.stringify({
				cart : cartItems,
    ...
    window.addEventListener('message', function(event) {
        ...
        async function playAudio(playdata){
            ...
            audioPlayer.on('end', function(){
                audioPlayer = null;
                $.post('http://ak47_khusbites/audioend', JSON.stringify({id:playId}));
            });
            ...
            while (audioPlayer != null) {
                $.post('http://ak47_khusbites/getvolume', JSON.stringify({
                    id: playId
                }), function(cb) {
                    if (audioPlayer != null) {
