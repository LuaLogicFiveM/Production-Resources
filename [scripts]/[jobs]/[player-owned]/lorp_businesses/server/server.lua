	...
	swapHook = exports.ox_inventory:registerHook('swapItems', function(payload)
		for k in pairs(Config.Shops) do
			if payload.fromInventory == k then
				TriggerEvent('lorp_businesses:refreshShop', k)
			elseif payload.toInventory == k and tonumber(payload.fromInventory) then
				TriggerClientEvent('lorp_businesses:setProductPrice', payload.fromInventory, k, payload.toSlot)
			end
		end
	end, {})
...
RegisterNetEvent('lorp_businesses:refreshShop', function(shop)
	Wait(250)
	...
	exports.ox_inventory:RegisterShop(shop, {
		...
		inventory = stashItems,
		locations = {
			Config.Shops[shop].shop.coords,
		}
	})
end)

RegisterNetEvent('lorp_businesses:setData', function(shop, slot, price)
	...
	TriggerEvent('lorp_businesses:refreshShop', shop)
end)
