...
local function WaitForCore(timeout)
    ...
    if not Core then
        if Config.Debug then
            print(string.format("[^1ERROR^7][tw-plumber] Core not loaded after %d seconds", (timeout * 50) / 1000))
    ...
    CreateThread(function()
        if Config.Framework ~= 'vrp' and Config.Framework ~= 'standalone' then
            if not WaitForCore(100) then
                if Config.Debug then
                    print(string.format("[^1ERROR^7][tw-plumber] Failed to register server callback: ^5%s^7", eventName))
                end
                return
            end
        end
...
RegisterNetEvent(_event('triggerServerCallback'), function(eventName, requestId, invoker, ...)
    if not _G.serverCallbacks[eventName] then
        if Config.Debug then
            print(string.format("[^1ERROR^7][tw-plumber] Server Callback not registered!"))
...
function RegisterCallback(name, cbFunc)
    ...
    if Config.Framework == 'standalone' then
        -- Use custom callback system for standalone
        return
    end
...
function ExecuteSql(query, parameters)
    -- Standalone framework (no database - return empty result)
    if Config.Framework == 'standalone' then
        -- Return empty result for SELECT queries, return 1 for INSERT/UPDATE
        if query:lower():match("^select") then
            return {}
        else
