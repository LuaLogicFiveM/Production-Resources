...
function G.Server.HasMoneyAndRemove(src, type, count)
   local Player = QBCore.Functions.GetPlayer(src)
   local accountTypes = { cash = "cash", bank = "bank" }
...
function GetSharedGroupData(name)
   ...
   local job = QBCore.Shared.Jobs and QBCore.Shared.Jobs[name]
   if job then
      return job, "job"
   end

   local gang = QBCore.Shared.Gangs and QBCore.Shared.Gangs[name]
   if gang then
      return gang, "gang"
   end
...
local function GetJobGradeLabel(name, grade)
   ...
   local gradeData = groupData.grades[gradeKey]
   ...
   for _, data in ipairs(result) do
      if not seen[data.citizenid] then
         ...
         if not alreadyAdded then
            local gradeLevel, gradeName = extractGradeInfo(jobData.grade or jobData.grade_level or jobData.gradeLevel)
...
local function EmploymentSaveData(groupData, groupName, grade)
   ...
   if groupData and groupData.grades then
      gradeData = groupData.grades[gradeKey] or groupData.grades[resolvedGrade]
   ...
   return {
      ...
      grade = {
         ...
         label = gradeData and (gradeData.label or gradeData.name) or tostring(resolvedGrade),
         level = resolvedGrade
      },
      onduty = false,
...
function G.Server.SetPlayerJob(src, jobName, grade, oldJob)
   ...
   local gang = QBCore.Shared.Gangs and QBCore.Shared.Gangs[jobName]
   local isJob = job ~= nil
   local isGang = gang ~= nil
   local oldGang = oldJobName and QBCore.Shared.Gangs and QBCore.Shared.Gangs[oldJobName] or nil
   ...
   local success, err = pcall(function()
      if isJob then
         ...
         if oldGang and oldJobName ~= jobName then
            ...
            local hasDefaultGang = QBCore.Shared.Gangs and QBCore.Shared.Gangs[defaultGangName]
...
function G.Server.SavePlayer(citizenid, jobName, grade, oldJob)
   ...
   if oldJobName and QBCore.Shared.Gangs and QBCore.Shared.Gangs[oldJobName] and not isGang then
      local defaultGangName = "none"
      local defaultGang = QBCore.Shared.Gangs[defaultGangName]
      if defaultGang then
         local defaultSave = BuildEmploymentSaveData(defaultGang, defaultGangName, 0)
         MySQL.update.await(
            "UPDATE players SET gang = ? WHERE citizenid = ?",
            { json.encode(defaultSave), citizenid }
         )
      end
   end
...
function G.Server.AddVehicleToFrameworkGarage(src, data)
   ...
   local emptyVehicleDataJson = json.encode({})
