...
local function initGarages()
    for garageId, garage in pairs(config.garages) do 
        ...
        for categoryId, category in pairs(garage.categories) do 
            ...
            for _, vehicle in pairs(category.vehicles) do
                local newVehicle = {
                    ...
                    args = {garageMenuId = garageMenuId, model = vehicle.model, label = vehicle.label, mods = vehicle.mods, garageName = garage.name, categoryMenuId = categoryMenuId, plate = garage.plate, groupRequired = garage.groupRequired, gradeRequired = category.gradeRequired}
                }
        ...
        for locationId, location in pairs(garage.locations) do
            garagePoints[locationId] = lib.zones.box({
...
lib.registerContext({
	...
	options = {
	  ...
	  {
		title = locale("menu.removePearlescent"),
		icon = '3',
		onSelect = function()
