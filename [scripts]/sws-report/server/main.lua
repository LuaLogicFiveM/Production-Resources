...
local function resolvePlayerGroup(source)
    ...
    if Config.Permissions.aceGroups then
        ...
        local playerGroup = playerData and playerData.getGroup() or false

        if playerGroup then
            DebugPrint(("Fetched group from player: %s (%i)"):format(playerGroup, source))
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    if VoiceMessagesAvailable then
        if not Config.Discord.enabled or not Config.Discord.webhook then
            VoiceMessagesAvailable = false
