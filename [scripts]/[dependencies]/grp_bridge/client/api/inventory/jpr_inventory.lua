...
JprInventoryClientAPI.GetResourceName = function()
    return "jpr-inventory"
end

JprInventoryClientAPI.GetItemInfo = function(item)
    ...
    if invExport.GetItemInfo then
        ...
        if itemData then
            return {
                ...
                image = JprInventoryClientAPI.GetImagePath(item)
            }
        end
    end
...
JprInventoryClientAPI.Items = function()
...
JprInventoryClientAPI.HasItem = function(item, requiredCount)
...
JprInventoryClientAPI.GetItemCount = function(item)
    if not validateItem(item, "GetItemCount") then return 0 end

    return invExport:GetItemCount(item)
end

JprInventoryClientAPI.GetPlayerInventory = function()
...
JprInventoryClientAPI.GetImagePath = function(item)
    if not item then return "" end

    item = JprInventoryClientAPI.StripImageExtension(item)

    if GetResourceState('jpr-inventory') == 'started' then
        local resourceFile = LoadResourceFile("jpr-inventory", string.format("images/%s.png", item))
...
JprInventoryClientAPI.StripImageExtension = function(item)
    if not item then return "" end
    return item:gsub("%.png$", ""):gsub("%.jpg$", ""):gsub("%.jpeg$", ""):gsub("%.webp$", "")
end

JprInventoryClientAPI.CanCarryItem = function(item, count)
...
JprInventoryClientAPI.GetInventoryWeight = function()
...
JprInventoryClientAPI.GetInventoryMaxWeight = function()
