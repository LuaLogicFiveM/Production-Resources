...
Config.Models = {
    [DEVICE_TYPES.Camera] = `prop_spycam`,
    [DEVICE_TYPES.Sensor] = `ch_chint02_motionsensor`
}

-- if an item can be placed inside a route bucket
Config.TypeRouting = {
    [DEVICE_TYPES.Camera] = false,
    [DEVICE_TYPES.Sensor] = true
}

-- how far motion detectors scan
Config.MotionSensorDistance = 7.5

-- how long a motion sensor takes to reset before it detects movement again
Config.MotionSensorResetTime = 25 -- [[ seconds ]]

-- how close you have to be to the device to place it
Config.DevicePlaceDist = 2.0

-- how many angles you can turn the camera
Config.CameraMaxRotation = 65.0

Config.CheckWall = true
Config.BlockedMaterials = {
    -- None = 0x0,
    -- Default = 0x962C3F7B,
    -- Concrete = 0x46CA81E8,
    ...
    -- Not sure if we want to allow for cars?
...
Config.BlockedMaterialsLookup = {}

Citizen.CreateThread(function()
    for _, v in pairs(Config.BlockedMaterials) do
        if(v >= 2^31) then -- fucking lua moment, it took me like 30 min to find out why onluy some materials are working
            v = v - 2^32
        end

        Config.BlockedMaterialsLookup[math.floor(v)] = true
    end
end)
