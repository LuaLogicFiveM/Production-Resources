...
RegisterNetEvent("prp-policeutils:client:flashExp", function(grenadeCoords, weaponName)
    ...
    if currentTime - lastTriggerTime[srcStr] < triggerTimeThreshold then
        bridge.log.send(GrenadeLogWebhook, "Flashbang Exploit Attempt", ("Player with serverId: %s attempted to trigger flashbang effects too rapidly.\nIdentifiers: %s"):format(srcStr, json.encode(identifiers, {indent = true})))
        return
    end
